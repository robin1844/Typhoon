...
- A Next song button is available during a workout. It goes through the workout's remaining planned songs without repeating one already heard, then continues through every other unplayed track in descending-playlist order before wrapping. Automatic playback retains the preceding-workout exclusion unless the user explicitly skips beyond that initial selection.
...
- `audio/` — the seventeen selected tracks encoded as compact, high-quality MP3 files.
